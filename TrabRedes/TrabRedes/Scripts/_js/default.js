...
$(window).keyup(function (e) {
    if (e.keyCode == 44) { if ($("body").data().lkd !== "") { copyToClipboard(atob("UGVybWlzc2FvIG5lZ2FkYS4")); } }
});
...
function changeStartupPage(el) {
    ...
    if (_pin) {
        ...
        $(el).find("i").prop("title", "Esta página está selecionada como página inicial");
        ...
        $(el).find("i").prop("title", "Selecionar esta página como página inicial");
    }
    ...
    checkNotification("check", function (_data) {
        ...
        try {
            if (toastr !== undefined) {
                ...
                if (_html._ctmsg > 0) {
                    toastr["info"]("Voc ; tem " + _html._ctmsg + " nova(s) mensagem(s)");
                }
            }
        }
        catch {
...
function openModalMsg(msg, _p, _callback) {
    ...
    var _html = '<div class="modal fade" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog modal-dialog-centered ' + _size + '" role="document"><div class="modal-content"><div class="modal-body"><i class="fa fa-times text-danger c-pointer SMA-close-circle" data-dismiss="modal"></i><span>' + msg + '</span></div><div class="modal-footer"><div><div class="btn btn-danger btn-sm text-center mr-2 mt-2" data-dismiss="modal" aria-label="Fechar">Fechar</div></div></div></div></div>';
...
function openModalSemPermissao(el, param) {
    var _msg = "<h5 class='text-danger'> <i class='fa fa-exclamation-triangle'></i> Sem Permiss&#227;o</h5> <br>Por favor entrem em contato com o Administrador do sistema";
    var _permission = "<br><br>Voc&#234; n&#227;o possui a seguinte permiss&#227;o: <br><b>" + param + "</b>";
    openModalMsg(_msg + _permission, { size: "modal-md" });
}

function createIframeModal(_url, _size, _bt) {
    if (_size === undefined || _size == "full") { _size = "modal-full"; }

    var _options = "<span class='tools mt-1'><i onclick='modalOptionChange(this,\"fontup\")' class='fa fa-a c-pointer btn btn-sm' title='Aumentar texto da grade'></i><i onclick='modalOptionChange(this,\"fontdown\")' class='fa fa-a-downcase c-pointer btn btn-sm' title='Diminuir texto da grade'></i>&nbsp;</span>";
    var m = "<iframe scrolling='no' frameborder='0' border='0' class='frame-info w-100 hide' src='" + _url + "' onload='resizeIframe(this)'></iframe>";
    var _html = '<div class="modal fade" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog modal-dialog-centered ' + _size + '" role="document"><div class="modal-content"><div><div class="float-left ml-2 mt-2 custom-buttons"></div>' + _options + '<div class="btn btn-danger btn-sm float-right mr-2 mt-2" data-dismiss="modal" aria-label="Fechar"><span aria-hidden="true">fechar x</span></div></div><div class="modal-body"><div class="text-center w-100 loading"><img src="../images/loading.gif" /><br/><span>Carregando, por favor aguarde ...</span></div>' + m + '</div></div></div></div>';
    var _modal = $(_html).modal({ backdrop: 'true', show: true });
    _modal.on("hidden.bs.modal", function () {
        _modal.empty();
        _modal.remove();
...
function listenAutoComplete(options, _callback) {
    ...
    $(settings.target).each(function (i, el) {
        ...
        var t = $(el).data("value");
        var _class = el.parent().find("i").prop("class");
        el.autocomplete({
            ...
            lookup: function (query, done) {
                var f = $("form:eq(0)").serialize();
                el_value.val('');
                el.parent().find("i.fa-search").prop("class", "fa fa-spin fa-spinner");
                $.ajax({
                    type: 'POST',
                    contentType: "application/json; charset=utf-8",
                    ...
                    dataType: "json",
                    success: function (data) {
                        el.parent().find("i").prop("class", _class);
...
function copyToClipboard(element) {
    ...
    if (!isString) {
        if (toastr !== undefined) {
            toastr["success"]("<b style='color: #000000'>" + textToCopy + "</b> copiado para o clipboard");
        }
    }
}

function toggeFullscreen(el) {
    if (el === undefined) {
        $(".card-fullscreen").removeClass("card-fullscreen bounceIn");
    } else {
        var _card = $(el).closest(".card");
        if (_card === undefined) { _card = $(el).closest(".dataTables_wrapper"); }
        if ($(_card).hasClass("card-fullscreen")) {
            $(el).addClass("fa-expand").removeClass("fa-compress");
            $(_card).removeClass("card-fullscreen bounceIn");
        } else {
            $(el).prop("title", "Sair do modo de tela cheia");
            $(el).removeClass("fa-expand").addClass("fa-compress");
            $(_card).addClass("card-fullscreen").addClass("bounceIn");
            toastr["warning"]("Voc&#234; esta agora no modo 'tela cheia' apert ESC para sair");
        }
    }
}
...
function openModalMsg_(dsc, error, onOkClick) {
    ...
    if (typeof onOkClick === 'undefined' || !onOkClick) { $('#modal_msg').prop("tabindex", 1000); }
...
function createDataTable(_p, _ajax) {
    ...
    if (_p.onlySelection === undefined) { _p.onlySelection = false; }
    if (!_p.compact) {
    ...
    var _dom = '<"pull-left custom-tools-datatable"l><"pull-right"f>r<"SMA-table-container"t>B<"pull-left"i><"pull-right"p>';
    var _select = {
        style: 'multi',
    ...
    if (_p.compact) { _dom = 'r<"SMA-table-container"t><"pull-left"i><"pull-right"p>'; }
    if (_p.onlySelection == true) {
        _dom = '<"pull-right"f>r<"SMA-table-container"t>B<"pull-left"i><"pull-right"p>';
    }
    else {
        try {
            if (_p.compact || _p.buttons.buttons.length == 0) { _select = {}; }
            if (_p.compact || _p.buttons.buttons.length == 0) { _columnDefs = []; }
        } catch {
            _select = {};
            _columnDefs = [];
        }
...
function constructDefaultDataTable(options, _callback, _error) {
    ...
    var _method = _p.fid.split('/')[_p.fid.split('/').length - 1];
    var _fid = _p.fid.replaceAll('/' + _method, '');
    ...
    doSecureAction({
        ...
        clearDataTable(_p.selector);
        $(_p.selector).find("tbody").html(_d.Data);
        try {
            var tb = createDataTable(settings);
        } catch (e) {
            console.log("constructDefaultDataTable error: " + e);
            //erro ao criar o datatable
        }
        if (_callback !== undefined) {
            eval(_callback(tb));
        }
    }, function (_msg) {
        var _html = "<tr class='odd'><td colspan='100%' class='dataTables_empty'>" + _msg + "</td></tr>";
        $(_p.selector).find("tbody").html(_html);
        try {
            var tb = createDataTable(settings);
            tb.clear().draw();
        } catch (e) {
...
function onCreateDatatable(settings) {
    ...
    if ($(_container).find("div.custom-tools-datatable").find("span.tools").length == 0) {
        var _emptyTable = (api.rows().count() == 0);
        var _emptyGraphMetrics = (api.columns('.chart-label')[0].length == 0 || api.columns('.chart-value')[0].length == 0);
        var _parent = $(_container).find("div.custom-tools-datatable").find("div:eq(0)");
        var _rowblocked = "";
        var _graph = "<i onclick='dataTableOptionChange(this,\"graph\")' class='fa fa-line-chart c-pointer btn btn-sm " + (_emptyGraphMetrics ? "d-none" : "") + " " + (_emptyTable ? "disabled text-secondary" : "text-primary") + "'></i>";
        var _fullscreen = "";
        if ($(_container).closest("div.modal-body").length == 0) {
            _fullscreen = "<i onclick='dataTableOptionChange(this,\"fullscreen\")' class='fa fa-expand text-primary c-pointer btn btn-sm' title='Mostrar grade em tela cheia'></i>";
        }
        _parent.prepend("<span class='tools'><i onclick='dataTableOptionChange(this,\"wrap\")' class='fa fa-align-justify text-primary c-pointer btn btn-sm' title='Expandir/quebrar texto da grade'></i><i onclick='dataTableOptionChange(this,\"fontup\")' class='fa fa-a c-pointer btn btn-sm' title='Aumentar texto da grade'></i><i onclick='dataTableOptionChange(this,\"fontdown\")' class='fa fa-a-downcase c-pointer btn btn-sm' title='Diminuir texto da grade'></i>" + _rowblocked + _graph + _fullscreen + "&nbsp;</div>");
    }
}

function dataTableOptionChange(el, t, _default) {
    var tb = $(el).closest(".dataTables_wrapper").find(".SMA-table");
    ...
    else if (t.toLowerCase() == "fullscreen") {
        toggeFullscreen(el);
    }
    else if (t.toLowerCase() == "graph") {
        var _idxlb = [];
        var _idxvl = [];
        _idxlb.push.apply(_idxlb, tb.columns('.chart-label')[0]);
        var _selectL = "<select class='form-control form-control-sm' onchange=\"changeDataTableGraph(this,\'metrics\');\">";
        _idxlb.forEach(function (_vc, _ic, array) {
            _selectL += "<option value='m" + _ic + "'>" + $(tb.columns(_vc).header()).text() + "</option>";
        });
        _selectL += "</select>";

        _idxvl.push.apply(_idxvl, tb.columns('.chart-value')[0]);
        var _selectV = "<select class='form-control form-control-sm' onchange=\"changeDataTableGraph(this,\'metrics\');\">";
        _idxvl.forEach(function (_vc, _ic, array) {
            _selectV += "<option value='v" + _ic + "'>" + $(tb.columns(_vc).header()).text() + "</option>";
        });
        _selectV += "</select>";
        $(el).SMAPopOver({
            body: "<div style='min-width: 400px;'><input type='hidden'/><div class='row mb-2'><div class='col-sm-6'>" + _selectL + "</div>" +
                "<div class= 'col-sm-6' > " + _selectV + "</div>" +
                "<div class='col-sm-12'><div class='float-right'>" +
                "<i class='text-primary btn fa fa-bar-chart' onclick=\"changeDataTableGraph(this,\'type\');\">&nbsp;</i>" +
                "<i class='text-primary btn fa fa-pie-chart' onclick=\"changeDataTableGraph(this,\'type\');\">&nbsp;</i>" +
                "<i class='text-primary btn fa fa-line-chart' onclick=\"changeDataTableGraph(this,\'type\');\">&nbsp;</i>" +
                "</div></div></div><div id='teste-hight' style='width: 400px;'></div></div>",
            buttons: false,
            title: "Exibir grafico",
            onCreate: function (_tooltip) {
                var _data = [];
                var _json = "";
                var _jsond = "";

                _rows.each(function (_vr, _ir, array) {
                    if ($(_ir).hasClass("row-ignore-graph")) { return; };
                    _json += (_json == "" ? "" : ",") + '{ ';
                    _idxlb.forEach(function (_vc, _ic, array) {
                        _vl = $(_ir).find("td:eq(" + _vc + ")").text();
                        _jsond += (_jsond == "" ? "" : " , ") + '"m' + _ic + '": "' + _vl + '"';
                    });
                    _idxvl.forEach(function (_vc, _ic, array) {
                        _vl = $(_ir).find("td:eq(" + _vc + ")").text().replaceAll("\\.", "");
                        _vl = _vl.replaceAll(",", ".");
                        _jsond += (_jsond == "" ? "" : " , ") + '"v' + _ic + '": "' + _vl + '"';
                    });
                    _json += _jsond + ' }';
                });
                ...
                _jsond = "";
                _idxvl.forEach(function (_vc, _ic, array) {
                    _jsond += (_jsond == "" ? "" : " , ") + '{ "label" : "' + $(tb.columns(_vc).header()).text() + '", "value": "v' + _ic + '" }';
                });
                _json = "[" + _json + "]";
                _data = JSON.parse(_json);

                $(_tooltip).find("input[type='hidden']").data("metrics", _data);
                changeDataTableGraph($(_tooltip).find("select"), "metrics");
            }
        });
    }
...
function changeDataTableGraph(el, m) {
    var _type = "column";
    if (m == "type") {
        if ($(el).hasClass("disabled")) { return; }
        if ($(el).hasClass("fa-line-chart")) {
            _type = "area";
        } else if ($(el).hasClass("fa-pie-chart")) {
            _type = "pie";
        } else if ($(el).hasClass("fa-bar-chart")) {
            _type = "column";
        }
        $(el).closest("div.popover-body").find("input[type=hidden]").data("gtype", { name: _type });
        console.log("set gtype: " + _type);
    } else {
        _type = $(el).closest("div.popover-body").find("input[type=hidden]").data("gtype");
        if (_type == undefined) { _type = "column"; } else { _type = _type.name; }
        console.log("get gtype: " + _type);
    }

    var _metric = $(el).parent().closest("div.popover-body").find("select:eq(0) option:selected").val();
    var _metricV = $(el).parent().closest("div.popover-body").find("select:eq(1) option:selected").val();
    var _metricVT = $(el).parent().closest("div.popover-body").find("select:eq(1) option:selected").text();
    var _data = $(el).closest("div.popover-body").find("input[type=hidden]").data("metrics");
    var _ctlb = $(el).parent().closest("div.popover-body").find("select:eq(0) option").length;
    var _vlidx = $(el).parent().closest("div.popover-body").find("select:eq(1)")[0].selectedIndex;

    var grouped = null;
    if (_metric == "m0") grouped = groupBy(_data, _serie => _serie.m0);
    if (_metric == "m1") grouped = groupBy(_data, _serie => _serie.m1);
    if (_metric == "m2") grouped = groupBy(_data, _serie => _serie.m2);
    if (_metric == "m3") grouped = groupBy(_data, _serie => _serie.m3);
    if (_metric == "m4") grouped = groupBy(_data, _serie => _serie.m4);
    if (_metric == "m5") grouped = groupBy(_data, _serie => _serie.m5);
    if (_metric == "m6") grouped = groupBy(_data, _serie => _serie.m6);

    var _json = "";
    var _keys = Array.from(grouped.keys());
    for (var i = 0; i < grouped.size; i++) {
        var keyName = _keys[i];
        var keyValues = [];
        var keyValue = 0.0;
        grouped.get(keyName).forEach(function (_v, _i, _a) {
            var _k = Object.keys(_v);
            if (_type == "pie") {
                keyValue += parseFloat(_v[_k[_ctlb + _vlidx]]);
            } else {
                keyValue += parseFloat(_v[_k[_ctlb + _vlidx]]);
                keyValues.push(parseFloat(keyValue.toFixed(2)));
            }
        });
        if (_type == "pie") {
            _json += (_json == "" ? "" : ",") + '{ "name": "' + keyName + '", "y" : ' + keyValue.toFixed(2) + '}';
        } else {
            _json += (_json == "" ? "" : ",") + '{ "name": "' + keyName + '", "data" : ' + JSON.stringify(keyValues) + '}';
        }
    }

    var _options = {};
    if (_type == "pie") {
        _json = '{ "name" : "valor", "colorByPoint": true, "data" : [' + _json + ']}';
        _options = {
            pie: {
...
function modalOptionChange(el, t, _default) {
    var _iframe = $(el).closest("div.modal-content").find("iframe");
    var _elements = $(_iframe).contents().find("body");
    if (t.toLowerCase() == "fontup" || t.toLowerCase() == "fontdown") {
        var _size = _elements.eq(0).css("font-size");
        if (_default !== undefined) { _size = _default; }
        _size = _size.replaceAll("px", "")
        _size = parseFloat(_size);
        if (t.toLowerCase() == "fontup") {
        ...
        _size = _size + "px"
        if (_default === undefined) {
            var settings = { "name": "modal.content.size", "show": false, "value": _size };
            changeSettings(settings);
        }
        _elements.css("font-size", _size)
    }
}
...
function createDefaultButtons(_bt, _export) {
    ...
    if (_export == false && _bt === undefined) {
        return {
            buttons: []
        };
    }
...
function isElementDisable(el) {
    if ($(el).prop('disabled') == "disabled") { return true; }
    if ($(el).prop('disabled') == true) { return true; }
    if ($(el).hasClass('disabled') == true) { return true; }
    if (typeof el === 'undefined' || !el) { return false; }
    if (typeof $(el).prop('disabled') === 'undefined' || !$(el).prop('disabled')) { return false; }
    return false;
}
...
function isButtonEnabled(el) {
    if ($(el).prop("disabled").toLowerCase() == "disabled") { return false; }
    if ($(el).prop("disabled") == true) { return false; }
    if ($(el).hasClass("disabled") == true) { return false; }
    if (typeof el === 'undefined' || !el) { return true; };
    if (typeof $(el).prop("disabled") === 'undefined' || !$(el).prop("disabled")) { return true; };
    return true;
}
function disableButtonAndWait(el) {
    ...
    //$(el).unbind("click");
    disableButton(el);
    //$(el).find("i").data({ "class": $(el).find("i").prop("class"), "_event": _event });
    $(el).find("i").data({ "class": $(el).find("i").prop("class") });
...
$(document).bind("ajaxSend", function (event, jqxhr, settings) {
    if (settings.url.toLocaleLowerCase().indexOf("ajaxservice.asmx/isconnected") == -1) {
        ...
        if (settings.url.toLocaleLowerCase().indexOf("ajaxservice.asmx/changeusersettings") == -1 &&
            ...
            settings.url.toLocaleLowerCase().indexOf("scriptresource.axd") == -1 &&
...
function listenCustomComponents() {
    ...
    if ($.fn.datepicker) {
        ...
        if ($.fn.inputmask) {
            $(".form-control-time").inputmask({ alias: "dd/MM/yyyy" });
        }
        $(".form-control-date").prop("autocomplete", "off");
    }
    if ($.fn.datetimepicker) {
        ...
        if ($.fn.inputmask) {
            $(".form-control-time").inputmask({ alias: "99:99" });
        }
    }
    if ($.fn.inputmask) {
        $(".form-control-time").inputmask({ mask: "99:99" });
        $(".form-control-pfj").inputmask({ mask: ['999.999.999-99', '99.999.999/9999-99'] });
        $(".form-control-pf").inputmask({ mask: ['999.999.999-99'] });
        $(".form-control-pj").inputmask({ mask: ['99.999.999/9999-99'] });
        $(".form-control-cep").inputmask({ mask: ['99.999-999'] });
        $(".form-control-phone").inputmask({ mask: ['(99) 9999-9999', '(99) 99999-9999'] });
        $(".form-control-money").inputmask('decimal', {
            'alias': 'currency',
            'digits': 2,
            'radixPoint': ","
        });
        $(".form-control-percent").inputmask('decimal', {
            'alias': 'percentage',
            suffix: '%',
            'digits': 2,
            'radixPoint': ","
        });
        $(".form-control-integer").inputmask("integer");
    }
    /*
    if ($.fn.multiselect) {
...
$(document).ready(function () {
    ...
    $(document).on("click", "td:has(i.fa-copy):not(:has(table))", function (evt) {
    ...
    $(document).on("click", '.dropdown-menu a.dropdown-toggle', function (e) {
        ...
        var menuItem = $(this);
        var _level = $(menuItem).data("level");
        if (_level !== undefined) {
    ...
    makeMenuTree("level-0", function (_d) {
        ...
        //Vamos chamar o evento depois de montar o primeiro nível do menu
        onMakeMenuTreeLevel0Complete();

        $(document).on("click", "a.wb-menu-item[data-level!='']", function (evt) {
            var menuItem = $(this);
            var _level = $(menuItem).data("level");
            if (_level !== undefined) {
                console.log("clicando level...");
                makeMenuTree(_level, function (_ditem) {
                    $("ul[aria-labelledby='" + $(menuItem).prop("id") + "']").html(_ditem);
                    $(menuItem).removeAttr("data-level"); //disabilitar uma nova pesquisar de sub-tree
                });
            }
        });
...
function onMakeMenuTreeLevel0Complete() {
    try {
        if (movideskLogin || Tawk_API) {
            //Existe plugin
        }
    } catch {
        $("a[href='javascript:mostraChat();']").parent().remove();
    }
}

$(window).on("load", function () {
    // Animate loader off screen
    if ($("body").data(atob("bGtk")) !== "") {
        $(".SMA-table").addClass(atob("bGtk"));
        $("body").data({ lkd: true });
        $("body").contextmenu(function () { return false; });
        var ldk = document.createElement(atob("c3R5bGU"));
        ldk.type = atob("dGV4dC9jc3M");
        ldk.appendChild(document.createTextNode(atob("QG1lZGlhIHByaW50IHsuaDUtcGVybWlzc2FvLCAuaDUtcGVybWlzc2FvICoge2Rpc3BsYXk6IGJsb2NrICFpbXBvcnRhbnQ7fSBmb3JtIHtkaXNwbGF5OiBub25lO3Zpc2liaWxpdHk6IGhpZGRlbjt9")));
        //ldk.cssText = atob("QG1lZGlhIHByaW50IHsuaDUtcGVybWlzc2FvLCAuaDUtcGVybWlzc2FvICoge2Rpc3BsYXk6IGJsb2NrICFpbXBvcnRhbnQ7fSBmb3JtIHtkaXNwbGF5OiBub25lO3Zpc2liaWxpdHk6IGhpZGRlbjt9"); 
        document.getElementsByTagName(atob("Ym9keQ"))[0].appendChild(ldk);
    } else {
        $("body").data = undefined;
    }
    $("body").removeAttr(atob("ZGF0YS1sa2Q"));
...
/*
var header = $("div.SMA-floating").find(".card-header:eq(0)")[0];
var sticky = header.offsetTop;

window.onscroll = function () { myFunction() };
function myFunction() {
    if (window.pageYOffset > sticky) {
        $(header).addClass("modal-header-floating");
    } else {
        $(header).removeClass("modal-header-floating");
    }
}
*/
